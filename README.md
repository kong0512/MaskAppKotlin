...
#### LiveData
 - 수명 주기를 인식하는 데이터 홀더 클래스로, 옵저버 패턴이 적용되어 데이터가 변경되면 트리거를 발생시켜 별도의 처리(주로 UI의 변경)을 할 수 있도록 함.
 - 액티비티/프래그먼트가 Stop 상태일때는 작동하지 않고, Destroy 상태일때는 자동으로 제거되며, 화면을 가로로 변경되는 등의 상태 변경에도 바로 최신의 데이터를 반영할 수 있도록 하는 등, 안드로이드 개발의 큰 시행착오 중 하나인 라이프사이클 관리를 비교적 쉽게 할 수 있음.
 - 
#### ViewModel
#### DataBinding
